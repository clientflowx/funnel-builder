          ...
          <div className="space-y-2 font-medium flex flex-col h-full justify-between">
            ...
            <Link href={`/${lastOption.key}`} onClick={() => setClose()}>
              <div
                ...
                } flex items-center  rounded-md leading-2 text-white opacity-70 ${
                  isOpen ? "" : "justify-center"
                } hover:opacity-100 transition-all text-sm p-2 m-2 mb-14 cursor-pointer`}
              >
                <div className="w-5 text-white">
                  {<lastOption.icon size={20} />}
                </div>
                <div
                  className={`ms-3 h-5 ${
                    lastOption.titleVisible ? "" : "hidden"
                  } ${isOpen ? "" : "hidden"}`}
                >
                  {lastOption.title}
                </div>
