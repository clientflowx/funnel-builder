          ...
          <PopoverContent className="bg-black text-white border border-gray-800">
            {isSignedIn ? (
              <div className="flex flex-col w-full items-start justify-center gap-5">
                ...
                <div className="w-full text-center bg-gray-800 text-xs p-3 rounded-md">
                  Member since {user?.createdAt?.getFullYear()}
                </div>
                <div className="w-full flex flex-col items-start justify-between gap-2">
                  <Link
                    href="/profile"
                    className=" cursor-pointer text-sm font-medium opacity-90 hover:opacity-100 transition-all"
                  >
                    User settings
                  </Link>
                  <div className=" cursor-pointer text-sm font-medium opacity-90 hover:opacity-100 transition-all">
                    <SignOutButton
                      signOutCallback={() => setIsSignedIn(false)}
                    />
                  </div>
                </div>
              </div>
